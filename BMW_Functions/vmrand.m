%% Revised from vmrand.m (Dylan Muir <muir@hifo.uzh.ch>, 2012)


%%

function [tfVMVariates] = vmrand(fMu, fKappa, range, varargin)
...
fMu=scalemu(fMu, range);
fKappa=scalek(fKappa, range);
...
% scale variabtes
tfVMVariates=min(range)+(tfVMVariates+pi)*((max(range)-min(range))/2/pi);
...
function k=scalek(k, range)
r=(max(range)-min(range))/2/pi;
% k2sd
sd=sqrt(-2*log(besseli(1,k)./besseli(0,k)));
...
function mu=scalemu(mu, range)
mu=-pi+(mu-min(range))*(2*pi/(max(range)-min(range)));
