...
%
% Define the Category-Only (with Capacity) model
% ------------
% Output=Category_Only_with_Capacity(param, Data, Input)
%
% ## Theory ##
% This model assumed that there's a fixed item-based working memory
% capacity. And when the set size is smaller than the capacity, all of the 
% responses are made based on the categorical working memory 
% representations. The responses follow Von Mises distributions that 
% centered around the categories.
%
% ## Input ##
% check the manual for details (BMW('manual'))
%
% - param
% kappa_c, K (, kappa_r, bias, s)
%
% - Data
...
if ~strcmp(Input.Output,'Prior')
    ...
    if continuous==1
        ...
        for i_error=1:length(errors_c)
            ...
            if any(strcmp(Input.Variants,'ResponseNoise'))
                ...
                if K<N % Beyond capacity
                    p_error(i_error)=(1-K/N)*1/(error_range(2)-error_range(1))+(K/N)*besseli(0,conv_1_c)./(2*pi*besseli(0,kappa_c)*besseli(0,kappa_r));
                else
                    p_error(i_error)=besseli(0,conv_1_c)./(2*pi*besseli(0,kappa_c)*besseli(0,kappa_r));
                end
            else
                if K<N
                    p_error(i_error)=(1-K/N)*1/(error_range(2)-error_range(1))+(K/N)*exp(kappa_c.*cosd(error0_c))./(2*pi*besseli(0,kappa_c));
                else
                ...
                if N==1
                    ...
                    if any(strcmp(Input.Variants,'ResponseNoise'))
                        for i_nt=1:N-1
                            ...
                            if K<N % beyond capacity
                                p_temp_NT=p_temp_NT+(1-K/N)*1/(error_range(2)-error_range(1))+...
                                    (K/N)*(besseli(0,conv_1_c_nt)./(2*pi*besseli(0,kappa_c)*besseli(0,kappa_r)));
                        ...
                        for i_nt=1:N-1
                            error0_nt_c=errors_nt_c(i_error,i_nt)+bias;
                            if K<N
                                p_temp_NT=p_temp_NT+(1-K/N)*1/(error_range(2)-error_range(1))+(K/N)*(exp(kappa_c.*cosd(error0_nt_c))./(2*pi*besseli(0,kappa_c)));
                            else
        ...
        for i=1:length(errors_c)
            p_T(i)=(1-s)*(p_error_c(SS_range==SS(i),error_range==errors_c(i),1));
        end
        if any(strcmp(Input.Variants,'Swap'))
            for i=1:length(errors_nt_c)
                if SS(i)==1
                    ...
                    for j=1:SS(i)-1
                        p_NT(i)=p_NT(i)+s/(SS(i)-1)*(p_error_c(SS_range==SS(i),error_range==errors_nt_c(i,j), 1));
                    end
                end
            end
        end
...
kappa_c=param(2); % categorical memory precision
% Gamma prior for categorical memory precision
p0(2)=gampdf(kappa_c,3,5);
Nparam=2;
if ~isfield(Input,'Variants') % No Variants
    Input.Variants={};
end
