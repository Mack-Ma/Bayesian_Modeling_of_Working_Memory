...
SampleSeed=1000*Nparams; % Monte Carlo seed
...
for i_N=1:length(SS_range)
    ...
    for i_error=1:length(error_range)
        ...
        p_error0(:,i_error,1)=besseli0_fast(conv_kappa)./(2*pi*besseli0_fast(kappa)*besseli0_fast(kappa_r));
        p_error0_c(:,i_error,1)=besseli0_fast(conv_kappa_c)./(2*pi*besseli0_fast(kappa_c)*besseli0_fast(kappa_r));
    end
    p_error(i_N,:,1)=mean(p_error0,1);
    p_error_c(i_N,:,1)=mean(p_error0_c,1);
